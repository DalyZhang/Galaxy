...
RJO.Intr.SwiperContainer = $("#info-container");
...
function gotoRecr(){
	window.open("recruit.html", "_self");
}
function gotoMenu(){
...
function refreshBoxPos(){
	...
	if(boxWidth>boxHeight){
		RJO.Intr.SwiperContainer.height(Math.min(boxHeight,400));
  		var height = RJO.Intr.SwiperContainer.height();
		RJO.Intr.SwiperContainer.height(height);
	}else{
		RJO.Intr.SwiperContainer.width(Math.min(boxWidth,400));
  		var width = RJO.Intr.SwiperContainer.width();
		RJO.Intr.SwiperContainer.height(width);
	}
  	var width = RJO.Intr.SwiperContainer.width();
	
	RJO.Intr.SwiperContainer.css({"left":(boxWidth-width)/2+"px"});
	RJO.Intr.SwiperContainer.css({"bottom":"0px"});
}
